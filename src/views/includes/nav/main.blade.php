<li{!! is_current_resource(['cms-article','cms-library','cms-article-family','cms-section','cms-category','cms-attachment-family']) !!}>
    ...
    <ul class="sub-menu">
        @if(is_allowed('cms-article', 'access'))
            <li{!! is_current_resource('cms-article') !!}><a href="{{ route('cmsArticle', [base_lang2()->id_001]) }}"><i class="fa fa-file-text-o"></i>{{ trans_choice('pulsar::pulsar.article', 2) }}</a></li>
        @endif
        @if(is_allowed('cms-category', 'access'))
            <li{!! is_current_resource('cms-category') !!}><a href="{{ route('cmsCategory', [base_lang2()->id_001]) }}"><i class="fa fa-list-ol"></i>{{ trans_choice('pulsar::pulsar.category', 2) }}</a></li>
        @endif
