...
class CmsCreateTableArticle extends Migration {
    ...
    public function up()
    {
        Schema::create('013_355_article', function(Blueprint $table)
        {
            ...
            $table->string('section_id_355', 30);
            $table->integer('family_id_355')->unsigned()->nullable(); // element to set default article configuration
            $table->tinyInteger('status_id_355')->unsigned();  // 0 = draft 1 = publish
            $table->integer('publish_355')->unsigned();     // date when will be publish
            $table->string('publish_text_355');             // date publish in text format
