...
class Article2Controller extends Controller
{
    ...
    public function storeCustomRecord($parameters)
    {
        ...
        $article = Article::create([
            ...
            'publish_355'       => $this->request->input('publish')? \DateTime::createFromFormat(config('pulsar.datePattern') . ' H:i', $this->request->input('publish'))->getTimestamp() : (integer)date('U'),
            'publish_text_355'  => $this->request->input('publish')?  $this->request->input('publish') : date(config('pulsar.datePattern') . ' H:i'),
            'date_355'          => $this->request->input('date')? \DateTime::createFromFormat(config('pulsar.datePattern'), $this->request->input('date'))->getTimestamp() : null,
